    ...
    stage('job1') {
      ...
      steps {
        dockerNode(image: 'docker.io/jenkinsci/slave:latest') {
          ...
          checkout([$class: 'GitSCM', branches: [[name: '*/${BRANCH}']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CleanBeforeCheckout']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/jagular-spb/gridu-mvn.git']]])
          script {
            res = sh (
              script: "mkdir -p ~/.m2 && \
